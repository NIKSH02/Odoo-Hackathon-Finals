...
const BookingsManagement = () => {
  ...
  const dummyBookings = [
    { 
      ...
      venue: { name: 'Sports Complex Ltd', id: 'venue1' }, 
      court: { name: 'Basketball Court 1', id: 'court1' }, 
    ...
    { 
      ...
      venue: { name: 'Elite Sports Center', id: 'venue2' }, 
      court: { name: 'Tennis Court A', id: 'court2' }, 
    ...
    { 
      ...
      venue: { name: 'City Recreation Hub', id: 'venue3' }, 
      court: { name: 'Football Field', id: 'court3' }, 
  ...
  const dummyFacilities = [
    {
      ...
      photos: [
        { url: '/api/placeholder/400/300', caption: 'Main entrance', isMainPhoto: true },
    ...
    },
    {
      ...
      photos: [
        { url: '/api/placeholder/400/300', caption: 'Facility overview', isMainPhoto: true },
  ...
  const tabs = [
    ...
    { id: 'facilities', name: 'Facility Approvals', count: facilities.filter(f => f.status === 'pending').length }
  ];
  ...
  const handleApprovalSubmit = async (action, comments) => {
    ...
    console.log(`${action} facility ${selectedFacility.id} with comments:`, comments);
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-black">Bookings & Facilities Management</h1>
        <p className="text-gray-600 mt-2">Monitor bookings and approve facility registrations</p>
      </div>
...
const BookingsTable = ({ bookings, onViewBooking, getStatusColor }) => (
          ...
          {bookings.map((booking) => (
            <tr key={booking.id} className="hover:bg-gray-50">
              <td className="px-6 py-4 whitespace-nowrap">
                <div>
                  <div className="text-sm font-medium text-black">#{booking.id}</div>
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <div>
                  <div className="text-sm font-medium text-black">{booking.venue.name}</div>
                  <div className="text-sm text-gray-500">{booking.court.name}</div>
                </div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                <div>
                  <div className="text-sm font-medium text-black">{booking.bookingDate}</div>
                  <div className="text-sm text-gray-500">{booking.timeSlot.startTime} - {booking.timeSlot.endTime}</div>
                </div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
...
const FacilitiesTable = ({ facilities, onViewFacility, onApprovalAction, getStatusColor }) => (
  <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
    <div className="overflow-x-auto">
      <table className="w-full">
        <thead className="bg-gray-50 border-b border-gray-200">
        ...
        <tbody className="divide-y divide-gray-200">
          {facilities.map((facility) => (
            <tr key={facility.id} className="hover:bg-gray-50">
              <td className="px-6 py-4 whitespace-nowrap">
                <div>
                  <div className="text-sm font-medium text-black">{facility.name}</div>
                  <div className="text-sm text-gray-500">{facility.address.city}, {facility.address.state}</div>
                </div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                <div>
                  <div className="text-sm font-medium text-black">{facility.owner.name}</div>
                  <div className="text-sm text-gray-500">{facility.owner.email}</div>
                </div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                <div>
                  <div className="text-sm font-medium text-black">${facility.startingPrice}/hour</div>
                  <div className="text-sm text-gray-500">{facility.sportsSupported.length} sports</div>
                </div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                <span className={`px-2 py-1 text-xs font-medium rounded-full capitalize ${getStatusColor(facility.status)}`}>
                  {facility.status}
                </span>
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
